...
export default function AdminSolutionsPage() {
  ...
  const [modalState, setModalState] = useState<{ mode: 'create' | 'edit'; item?: Solution } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Solution | null>(null);

  const { data: solutions, isLoading } = useQuery({ queryKey: ['admin-solutions'], queryFn: () => solutionsApi.listAdmin() });

  const { register, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<SolutionSchemaValues>({
    resolver: zodResolver(solutionSchema),
  });
  const content = watch('content');

  function openCreate() {
    reset({ name: '', slug: '', category: 'sme', thumbnail: '', summary: '', content: '', is_hot: false, status: 'active' });
    setModalState({ mode: 'create' });
  }

  function openEdit(item: Solution) {
    reset({
      name: item.name, slug: item.slug, category: item.category, thumbnail: item.thumbnail ?? '',
      summary: item.summary ?? '', content: item.content, is_hot: item.is_hot, status: item.status,
    });
  ...
  function onSubmit(values: SolutionSchemaValues) {
    saveMutation.mutate({ ...values, thumbnail: values.thumbnail || null, summary: values.summary || null });
  }

  const columns: TableColumn<Solution>[] = [
    { key: 'name', header: 'Tên giải pháp', render: (s) => s.name, sortAccessor: (s) => s.name },
    { key: 'category', header: 'Nhóm', render: (s) => CATEGORY_OPTIONS.find((c) => c.value === s.category)?.label },
    { key: 'is_hot', header: 'Hot', render: (s) => (s.is_hot ? <Badge tone="accent">HOT</Badge> : null) },
    { key: 'status', header: 'Trạng thái', render: (s) => <Badge tone={s.status === 'active' ? 'success' : 'neutral'}>{STATUS_OPTIONS.find((o) => o.value === s.status)?.label}</Badge> },
    {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="font-heading text-2xl font-bold text-neutral-900">Quản lý Giải pháp số</h1>
        <Button onClick={openCreate}><Plus className="h-4 w-4" /> Thêm giải pháp</Button>
      </div>

      <Table columns={columns} data={solutions ?? []} rowKey={(s) => s.id} isLoading={isLoading} />

      <Modal isOpen={modalState !== null} onClose={() => setModalState(null)} title={modalState?.mode === 'edit' ? 'Sửa giải pháp' : 'Thêm giải pháp'} maxWidthClassName="max-w-2xl">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <SelectField label="Nhóm" options={CATEGORY_OPTIONS} error={errors.category?.message} {...register('category')} />
            <SelectField label="Trạng thái" options={STATUS_OPTIONS} error={errors.status?.message} {...register('status')} />
          </div>
          <TextField label="Ảnh đại diện (URL)" placeholder="https://..." error={errors.thumbnail?.message} {...register('thumbnail')} />
          ...
          <RichTextEditor label="Nội dung chi tiết" value={content ?? ''} onChange={(html) => setValue('content', html, { shouldValidate: true })} error={errors.content?.message} />
      ...
      <Modal isOpen={deleteTarget !== null} onClose={() => setDeleteTarget(null)} title="Xác nhận xóa">
        <p className="text-neutral-900">Bạn có chắc muốn xóa giải pháp <strong>{deleteTarget?.name}</strong>?</p>
